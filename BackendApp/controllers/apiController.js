...
function extractNumeroLote(req) {
...
export async function enviarDELoteController(req, res) {
    try {
        const inicial = await recibirJsonYGenerarXmlInicial({
            body: req.body,
            usuarioId: 1,
...
export async function consultarLoteSifenController(req, res) {
    // Leer desde rawBody evita el redondeo si llega un numero mayor a Number.MAX_SAFE_INTEGER.
    const numeroLote = extractNumeroLote(req);
